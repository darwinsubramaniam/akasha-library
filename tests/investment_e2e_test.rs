...
fn test_investment_service() {
    ...
    let _ = test_await!(investment_service.add(&new_investment));
    let result = test_await!(investment_service.get_by_currency(&"DAR".to_string()));
    let all = test_await!(investment_service.get_all()).unwrap();
    dbg!(all);
...
fn test_investment_service_get_investments() {
    ...
    let result = test_await!(investment_service.get_all());
    let result = result.unwrap();
...
fn test_investment_by_currency() {
    let mut mock_investment_db_service = MockInvestmentDBService::default();
    let investment_service = InvestmentService::new(&mut mock_investment_db_service);
    let result = test_await!(investment_service.get_by_currency(&"SGD".to_string()));
    let result = result.unwrap();
    assert_eq!(result.len(), 2);

    let result = test_await!(investment_service.get_by_currency(&"MYR".to_string()));
    let result = result.unwrap();
    assert_eq!(result.len(), 1);
}
...
fn tes_service_filter_by_type() {
    ...
    let result = test_await!(investment_service.get_by_type(&InvestmentType::DEPOSIT));
    let result = result.unwrap();
    assert_eq!(result.len(), 2);

    let result = test_await!(investment_service.get_by_type(&InvestmentType::WITHDRAW));
    let result = result.unwrap();
    assert_eq!(result.len(), 1);
}
...
fn test_update_by_id() {
    ...
    let _ =
        test_await!(investment_service.update_by_id(&"1".to_string(), &new_investment));

    let result = test_await!(investment_service.get_by_id(id)).unwrap().unwrap();
...
fn test_delete_by_id(){
    ...
    let mut investment_service = InvestmentService::new(&mut mock_investment_db_service);
    let id = "1";
    let _ = test_await!(investment_service.delete_by_id(id));
    let result = test_await!(investment_service.get_by_id(id)).unwrap();
...
fn test_total_investment_by_type(){
    ...
    let result = test_await!(
        investment_service.total_by_type(&InvestmentType::DEPOSIT,&desired_fiat, &service)
    ).unwrap();
    assert!(result == (100.0 + (10.0/2.0)));

    let result = test_await!(
        investment_service.total_by_type(&InvestmentType::WITHDRAW,&desired_fiat, &service)
    ).unwrap();
    assert!(result == 10.0);
}
